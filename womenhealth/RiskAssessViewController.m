...
- (void)viewDidLoad
{
    ...
    [alert show];
    [[ResultManager sharedInstance]createPlistFile];
    //[self createPicker];
    [super viewDidLoad];
...
- (IBAction)nextArrowPressed:(id)sender {
   ...
   // [self setTitleToNil];
    ...
    if(theDataObject.riskCount == 1){
        ...
        NSString *e = [self.pickerViewArray objectAtIndex:selectedIndex];
        theDataObject.userEthnicity = e;
         [[ResultManager sharedInstance]saveResultWithValue:e  withKey:[NSString stringWithFormat:@"Risk Assessment: 0%d", theDataObject.riskCount]];
    }
        ...
        if(([self.ageField.text length]==0)||([self.ageField.text intValue]>100)){
            ...
            theDataObject.userage = [self.ageField.text integerValue];
             [[ResultManager sharedInstance]saveResultWithValue:self.ageField.text  withKey:[NSString stringWithFormat:@"Risk Assessment: 0%d", theDataObject.riskCount]];
        }
    }
...
- (IBAction)firstButtonPressed:(id)sender {
    ...
    if (theDataObject.riskCount <10) {
        [[ResultManager sharedInstance]saveResultWithValue:title withKey:[NSString stringWithFormat:@"Risk Assessment: 0%d", theDataObject.riskCount]];
    }
    else{
        [[ResultManager sharedInstance]saveResultWithValue:title withKey:[NSString stringWithFormat:@"Risk Assessment: %d", theDataObject.riskCount]];
    }
...
-(void)setTitleToNil{
    ...
    [self.fifthButton setTitle:@"" forState:UIControlStateNormal];
}
