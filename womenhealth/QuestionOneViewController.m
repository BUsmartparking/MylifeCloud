...
- (IBAction)pressYes:(id)sender {
    ...
    if (theDataObject.riskCount <10) {
        [[ResultManager sharedInstance]saveResultWithValue:title withKey:[NSString stringWithFormat:@"Question One: 0%d", theDataObject.questionCount]];
    }
    else{
        [[ResultManager sharedInstance]saveResultWithValue:title withKey:[NSString stringWithFormat:@"Question One: %d", theDataObject.questionCount]];
    }
